...
void pm_app_wifi_action_scan(void) {
    s_net_count = 0;
    s_scanning  = true;
    pm_c6_cmd_send_raw("{\"cmd\":\"wifi_scan\"}");
}
...
void pm_app_wifi_disconnect(void) {
    pm_c6_cmd_send_raw("{\"cmd\":\"wifi_disconnect\"}");
}
...
static void _render(void) {
    // TODO_LVGL: header (CONNECTED ssid@ip / DISCONNECTED),
    //           [SCAN][CONNECT][FORGET] buttons,
    //           scrollable list with rssi/channel/enc cols,
    //           cursor highlight at s_cursor.
...
static void _render(void) { _refresh_status(); _refresh_list(); }
...
static void _enter(void) {
    ...
    pm_log_i(TAG, "enter");
    pm_c6_cmd_send_raw("{\"cmd\":\"wifi_status\"}");
    _render();
}
