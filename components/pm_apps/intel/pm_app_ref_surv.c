...
static void _build_screen(void) {
    s_default_screen = pm_ui_default_screen("REF: SURV",
        "REF: SURV app — UI ready");
}
...
static void _enter(void)  {
    if (s_default_screen) lv_screen_load(s_default_screen); pm_log_i(TAG, "enter"); if (s_browser) pm_ref_browser_refresh(s_browser); }
