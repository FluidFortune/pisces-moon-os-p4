...
static int _gen_moves(move_t* out, int max_out) {
    ...
    for (int r = 0; r < 8; r++) for (int c = 0; c < 8; c++) {
        ...
        #define TRY(_DR,_DC) do { \
            if (_on_board((_DR),(_DC)) && _sign(s_board[(_DR)][(_DC)]) != side && n < max_out) { \
                out[n].fr=r; out[n].fc=c; out[n].tr=(_DR); out[n].tc=(_DC); \
                out[n].captured = s_board[(_DR)][(_DC)]; n++; \
            } \
        } while (0)
